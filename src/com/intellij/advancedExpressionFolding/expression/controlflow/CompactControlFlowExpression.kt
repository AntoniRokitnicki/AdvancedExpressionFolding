...
class CompactControlFlowExpression(element: PsiElement, textRange: TextRange) : Expression(element, textRange) {
    companion object {
        fun buildFoldRegions(element: PsiElement, group: FoldingGroup, descriptors: ArrayList<FoldingDescriptor>, textRange: TextRange) {
            descriptors.add(
                FoldingDescriptor(
                    element.getNode(),
        ...
        fun supportsFoldRegions(document: Document, textRange: TextRange): Boolean {
            return textRange.getStartOffset() > 0 && textRange.getEndOffset() < document.getTextLength() - 1 &&
                InterpolatedString.OVERFLOW_CHARACTERS.contains(
                    document.getText(TextRange.create(textRange.getStartOffset() - 1, textRange.getStartOffset()))
                ) &&
                InterpolatedString.OVERFLOW_CHARACTERS.contains(
    ...
    override fun supportsFoldRegions(document: Document, parent: Expression?): Boolean {
        return supportsFoldRegions(document, getTextRange())
    }
    ...
    override fun buildFoldRegions(element: PsiElement, document: Document, parent: Expression?): Array<FoldingDescriptor> {
        ...
        buildFoldRegions(
            element,
            FoldingGroup.newGroup(CompactControlFlowExpression::class.java.getName() + Expression.HIGHLIGHTED_GROUP_POSTFIX),
            descriptors,
            getTextRange()
        )
