...
class CompactControlFlowExpression(element: PsiElement, textRange: TextRange) :
    ...
    override fun buildFoldRegions(
        ...
        buildFoldRegions(
            ...
            FoldingGroup.newGroup(
                CompactControlFlowExpression::class.java.name +
    ...
    companion object {
        ...
        fun supportsFoldRegions(document: Document, textRange: TextRange): Boolean {
            return textRange.startOffset > 0 &&
                textRange.endOffset < document.textLength - 1 &&
                InterpolatedString.OVERFLOW_CHARACTERS.contains(
                    document.getText(
                        TextRange.create(textRange.startOffset - 1, textRange.startOffset)
                    )
                ) &&
                InterpolatedString.OVERFLOW_CHARACTERS.contains(
                    document.getText(
                        TextRange.create(textRange.endOffset, textRange.endOffset + 1)
                    )
                )
        }
    }
