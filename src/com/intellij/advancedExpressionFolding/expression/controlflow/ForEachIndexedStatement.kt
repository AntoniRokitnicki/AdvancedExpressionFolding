...
class ForEachIndexedStatement(
    ...
    private var element: PsiForStatement = element
    private var declarationTextRange: TextRange = declarationTextRange
    private var indexTextRange: TextRange = indexTextRange
    private var variableTextRange: TextRange = variableTextRange
    private var arrayTextRange: TextRange = arrayTextRange
    private var varSyntax: Boolean = varSyntax
    private var isFinal: Boolean = isFinal
    ...
    override fun buildFoldRegions(element: PsiElement, document: Document, parent: Expression?): Array<FoldingDescriptor> {
        ...
        val prefixRange = TextRange.create(getTextRange().getStartOffset(), getTextRange().getStartOffset() + 1)
        var prefix = document.getText(prefixRange)
        if (AdvancedExpressionFoldingSettings.getInstance().getState().getCompactControlFlowSyntaxCollapse() && prefix == "(") {
            prefix = ""
        }
        if (varSyntax) {
            descriptors.add(
                FoldingDescriptor(
                    element.getNode(),
                    TextRange.create(getTextRange().getStartOffset(), indexTextRange.getStartOffset()),
                    group,
