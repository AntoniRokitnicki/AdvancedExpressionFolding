...
public class FieldShiftBuilder {
    ...
    public static FieldShiftBuilder map(UserData2 source, BuilderFieldShiftBuilder builder, UserDataRecord record) {
        BuilderFieldShiftBuilder builder1 = builder
                .username(record[8:".username()"]);
        var builder2 = builder
                .active(source[9:".isActive()"]);
        return FieldShiftBuilder.builder().username(record.[10:"userIdentifier()"]).username(changer(record[11:".username()"]))
                .username(source[12:".getUsername()"]).username(builder.username("a").build()[13:".getUsername()"])
                .username(source.[14:"getUsername()"] + "1")
                .active(source[15:".isActive()"]).userIdentifier(source[16:".getUserIdentifier()"])
                .child(FieldShiftBuilder.builder()
                        .child(builder1
                                .userIdentifier(source.[18:"getUsername()"])
                                .username(source[19:".getUsername()"])
                                .build()[17:".getChild()"])
                        .active(builder.build()[20:".isActive()"])
                        .username(builder.build()[21:".getUsername()"])
                        .userIdentifier(record[22:".userIdentifier()"])
                        .build())
                .child(builder2
    ...
    public static FieldShiftBuilder mapSimple(FieldShiftBuilder source) {
        return FieldShiftBuilder.builder()
                .username(source[40:".getUsername()"])
                .userIdentifier(source[41:".getUserIdentifier()"])
                .build();
    }

    public static FieldShiftBuilder mapUserDataAllFields(UserDataRecord source) {
        return FieldShiftBuilder.builder()
                .username(source[44:".username()"])
                .active(source[45:".active()"])
                .userIdentifier(source[46:".userIdentifier()"])
                .build();
    }
    ...
    public static class UserData2 {
        ...
        UserData2(String username, boolean active, String userIdentifier) {
            this.username = [50:"username"];
            this.active = [51:"active"];
            this.userIdentifier = [52:"userIdentifier"];
        }
