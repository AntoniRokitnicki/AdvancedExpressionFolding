...
class FindMethodsWithDefaultParametersActionIntegrationTest {
    ...
    fun `find methods action populates usage view`() {
        ...
        context.runIdeWithDriver().useDriverAndCloseIde {
            ...
            waitForUsageCompletion(usageBridge)
            ...
            check(events.map { it.text }.containsAll(listOf("Processing file 1 of 2", "Processing file 2 of 2"))) {
    ...
    private fun waitForUsageCompletion(usageBridge: FindUsageCustomViewTestBridge, timeout: Duration = 10.seconds) {
        val deadline = System.currentTimeMillis() + timeout.inWholeMilliseconds
        while (!usageBridge.isSearchFinished()) {
            if (System.currentTimeMillis() > deadline) {
                error("Timed out waiting for usage view to finish")
            }
            Thread.sleep(100)
        }
    }
